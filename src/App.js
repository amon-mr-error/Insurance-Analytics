...
import React, { useState } from 'react';
import { 
  BarChart, PieChart, Bar, XAxis, YAxis, Tooltip, Legend, Pie 
} from 'recharts';
import { 
  Shield, Users, TrendingUp, AlertOctagon, Search, Bell, Menu 
} from 'lucide-react';
import './App.css';
...
const InsuranceDashboard = () => {
  ...
  return (
    <div className="flex h-screen bg-gray-100">
      {/* Sidebar */}
      <div 
        className={`fixed inset-y-0 left-0 z-20 w-64 bg-white p-4 transform md:relative md:translate-x-0 transition-transform ${
      ...
      <div className="flex-1 flex flex-col">
        ...
        <div className="bg-white p-4 flex justify-between items-center shadow">
          <div className="flex items-center md:hidden">
            <Menu className="text-gray-500 cursor-pointer" onClick={() => setIsSidebarOpen(true)} />
          </div>
          <div className="flex items-center bg-gray-100 p-2 rounded-lg flex-1 md:ml-4">
            <Search className="text-gray-500 mr-2" />
            <input 
              type="text" 
              placeholder="Search customers, policies, claims..." 
        ...
        <div className="p-4 md:p-6 overflow-y-auto">
          {activeTab === 'overview' && (
            <div className="dashboard-grid grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {overviewKPIs.map(kpi => (
                <div 
                  key={kpi.title} 
                  className={`bg-white p-5 rounded-lg shadow border-l-4 border-${kpi.color}-500`}
...
<div className="bg-white p-5 rounded-lg shadow col-span-3">
  ...
  <div className="overflow-x-auto">
    <div className="pie-chart-container">
      <PieChart width={300} height={300}> {/* Adjust the width and height here */}
        <Pie 
          data={riskDistributionData} 
          dataKey="value"
          nameKey="category"
        ...
        <Tooltip />
          ...
          {activeTab === 'customers' && (
            <div className="space-y-4">
              ...
              {customerData.map(customer => (
                <div key={customer.id} className="bg-white p-5 rounded-lg shadow">
                  <div className="font-semibold">{customer.name}</div>
                  <div className="text-sm text-gray-500">Age: {customer.age}</div>
                  <div className="text-sm text-gray-500">Risk Score: {customer.riskScore}</div>
                  <div className="text-sm text-gray-500">Premium: {customer.premium}</div>
                </div>
              ))}
            </div>
          )}
          ...
          {activeTab === 'claims' && (
            <div className="space-y-4">
              ...
              {claimsData.map(claim => (
                <div key={claim.id} className="bg-white p-5 rounded-lg shadow">
                  <div className="font-semibold">Customer: {claim.customer}</div>
                  <div className="text-sm text-gray-500">Claim Amount: {claim.amount}</div>
                  <div className="text-sm text-gray-500">Status: {claim.status}</div>
                  <div className="text-sm text-gray-500">Fraud Risk: {claim.fraudRisk}</div>
                </div>
              ))}
            </div>
          )}
          ...
          {activeTab === 'fraud' && (
            <div className="space-y-4">
              <h2 className="text-lg font-semibold mb-4">Fraud Indicators</h2>
              {fraudIndicators.map(indicator => (
                <div key={indicator.indicator} className="bg-white p-5 rounded-lg shadow">
                  <div className="font-semibold">{indicator.indicator}</div>
                  <div className="text-sm text-gray-500">Count: {indicator.count}</div>
                  <div className="text-sm text-gray-500">Severity: {indicator.severity}</div>
                </div>
              ))}
            </div>
          )}
